...
module.exports = {
  ...
  async execute(invite) {
    try {
      ...
      const errorEmbed = new EmbedBuilder().setColor("Red").setTitle(`> ❌ Error invite delete`).setDescription(`\`\`\`${error}\`\`\``).setFooter(`Error dari server ${interaction.guild.name}`).setTimestamp();

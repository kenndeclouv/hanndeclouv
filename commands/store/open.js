        ...
        try {
            ...
            if (!channel) {
                return interaction.editReply({
                    content: "❌ | Channel untuk open/close store tidak ditemukan atau belum diatur.",
                    ephemeral: true
                });
            }

            const nameFormat = setting.openChannelNameFormat || "🟢・store-open";

            const embedData = setting.openChannelMessageFormat?.[0];
            let embed = null;
            ...
            return interaction.editReply({
                content: "✅ | Toko berhasil dibuka sesuai pengaturan.",
                ephemeral: true
            });

        } catch (error) {
            console.error("❌ Error during store open:", error);

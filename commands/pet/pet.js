const { SlashCommandBuilder, EmbedBuilder, SelectMenuBuilder, WebhookClient } = require("discord.js");
const { checkCooldown, checkPermission } = require("../../helpers");
      ...
      subcommand
        ...
        .addStringOption((option) =>
          option
            ...
            .addChoices({ name: "Common", value: "common" }, { name: "Rare", value: "rare" }, { name: "Epic", value: "epic" }, { name: "Legendary", value: "legendary" })
            .setRequired(true)
        )
    ...
    try {
      ...
      if (subcommand === "leaderboard") {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            leaderboard.length
              ? leaderboard.map((pet, index) => `${index + 1}. <@${pet.userId}> ${pet.pet.icon} ${pet.pet.rarity} ${pet.pet.name}, level ${pet.level}`).join("\n")
